...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Selecciona el contenedor principal del producto (título + botón)
    var productContainer = document.querySelector("[data-store^='product-info']");

    if (productDescription && productContainer) {
        // Mover la descripción justo después del contenedor principal
        productContainer.parentNode.appendChild(productDescription);

        // Mostrar la descripción
        productDescription.style.display = "block";

        // Márgenes laterales y centrado
        productDescription.style.margin = "20px auto";
        productDescription.style.maxWidth = "800px";
        productDescription.style.padding = "0 15px";
    }
});
</script>
{% endraw %}
